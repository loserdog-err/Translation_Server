...
public class AnswerServiceImpl extends DAOSupportImpl<Answer> implements AnswerService
{
	 ...
	 * 设置此答案为最佳答案
	 */
	public boolean setBestAnswer(int answerId)
	{
		try
		{
			Answer answer = getById(answerId);
			...
			/**
			 * 维护关联关系 1:设置此答案所归属的问题已解决
			 * 2:将问题所对应的悬赏积分奖励给最佳答案的用户
			 * 3:扣除提问者的积分4：用户所属团队得到相应的积分 5：添加消息提醒给
			 * 收藏者，回复者，最佳回答者
			 */
			...
			int bestAnswerCount = queryCount(
					"select count(*) from Answer where userId=? and isBest=?",
					new Object[] { user.getId(), true });// 得到用户最佳答案的数量
			if (answerCount > 0)
			{
				float correctRate = (int) (((float) bestAnswerCount / (float) answerCount) * 100.0f) / 100.0f*100;
				user.setCorrectRate(correctRate);
			}
			JpushClientUtils.sendPush(JpushClientUtils.buildPushObject_all_alias_alert(
					user.getUsername(), "恭喜您的答案在问题: " + question.getTitle() + " 中被采纳,增加积分: "
			...
			{
				UserRemind userRemind = new UserRemind(UserRemind.TYPE_FOLLOW_QUESTION_SOLVE,
						answer, user, new Date());
				userRemindService.save(userRemind);
				JpushClientUtils.sendPush(JpushClientUtils.buildPushObject_all_alias_alert(
			...
			{
				UserRemind userRemind = new UserRemind(UserRemind.TYPE_ANSWER_QUESTION_SOLVE,
						answer, answerer, new Date());
				userRemindService.save(userRemind);
				JpushClientUtils
						.sendPush(JpushClientUtils.buildPushObject_all_alias_alert(
								answerer.getUsername(), "您回复过的问题 " + question.getTitle()
										+ " 已有最佳答案，快去看看吧"));
			}
			if (bestAnswer != null)
			{
